...
\begin{longtable}{p{14cm}>{\raggedleft\arraybackslash}p{4cm}}
\textbf{Research Scientist} & \textbf{Jul 2025 – Nov 2025} \\
\textsc{UW Medicine Diabetes Institute – Bjornstad Lab} & \textsc{Seattle - USA} \\
\multicolumn{2}{p{18cm}}{%
\begin{itemize}
 \item Applied Cox proportional hazards models and differential expression analysis to metabolomic data to quantify diabetic complication risk and cardio-kidney-metabolic syndrome progression in youth-onset type 2 diabetes participants.
 \item Performed a network analysis using MetaboAnalyst to identify key metabolic pathways associated with CKM progression on youth-onset type 2 diabetes participants.
\end{itemize}
} \\
\\
...
```{r}
#| name: peer_review
peer_reviews <- get_cv_sheet('reviews') |>
  mutate(
...
#| results: asis
make_review_table(peer_reviews)
```
