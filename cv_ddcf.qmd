...
\vspace{1pt}
\begin{itemize}
\item \textbf{Programming:} SQL (MySQL), Python, R programming
\item \textbf{Cloud Computing:} HPC (Slurm), Linux environments (bash)
\item \textbf{Tools:} VS Code, Jupyter Lab, Quarto, Markdown, \LaTeX, Shiny (R), ArcGIS
\item \textbf{Version control:} Git (GitHub, Bitbucket)
\item \textbf{Libraries and packages:} \texttt{tidyverse}, \texttt{ggplot2}, \texttt{meta}/\texttt{metafor}, \texttt{lme4}, \texttt{survival}, \texttt{geepack}, \texttt{gam}, \texttt{lavaan}, \texttt{sf}, \texttt{rJAGS}, \texttt{rstan}, Pandas, Matplotlib, NumPy, MR-BRT (GBD Study estimation), amongst other tools.
\end{itemize}
\end{minipage}
